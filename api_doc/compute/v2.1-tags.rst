...
.. swagger:tag:: multiple-create
   ...
   servers withor without a reservation ID.
...
.. swagger:tag:: os-server-actions-add-v2.1
   ...
   Permits all users to list available server actions for a
   specifiedserver. Permits administrators to get details for a
   specifiedserver action for a specified server.
...
.. swagger:tag:: admin-password
   ...
   serverto change the password of the administrative user.
...
.. swagger:tag:: server-ips
   ...
   Lists the IP addresses assigned to an instance or shows details
   fora specified IP address.

.. swagger:tag:: server-metadata-v2.1
   :synopsis: Server metadata (servers, metadata)

   Lists metadata, creates or replaces one or more metadataitems, and
   updates one or more metadata items for a server.
   
   Shows details for, creates or replaces, and updates a
   specifiedmetadata item, by key, for a server.
...
.. swagger:tag:: flavor-extra-specs
   ...
   Lists, creates, deletes, and updates the extra-specs or keys for
   aflavor.
...
.. swagger:tag:: compute_images-v2.1
   ...
   An image is a collection of files that you use to create andrebuild
   a server. By default, operators provide pre-builtoperating system
   images. You can also create custom images. SeeCompute server
   actions.
   
   If you set the image size policy in the ``policy.json``file, the
   ``OS-EXT-IMG-SIZE:size``extended attribute isvisible.
...
.. swagger:tag:: agents
   ...
   Creates, lists, updates, and deletes guest agent builds. Use
   guestagents to access files on the disk, configure networking, or
   runother applications or scripts in the guest while the agent runs.
   Thishypervisor-specific extension is not currently enabled for KVM.
   Useof guest agents is possible only if the underlying service
   provideruses the Xen driver.
...
.. swagger:tag:: aggregates
   ...
   togroups of compute nodes. Aggregates are only visible to the
   cloudprovider.
...
.. swagger:tag:: os-assisted-volume-snapshots-v2.1
   ...
   An internal snapshot that lacks storage such as NFS or GlusterFScan
   use an emulator/hypervisor to add the snapshot feature.
...
.. swagger:tag:: floating-ip-dns
   ...
   Manages DNS records associated with IP addresses allocated by
   thefloating IPs extension. Requests are dispatched to a DNS
   driverselected at startup.
...
.. swagger:tag:: networks
   ...
   Adds network to a project, disassociates a network from a
   project,and disassociates a project from a network.
...
.. swagger:tag:: quota-sets
   ...
   quotasfor a project and view and update default quotas.
...
.. swagger:tag:: services
   ...
   Lists, enables, disables, and deletes Compute services in allhosts.
